...
<?php
    ...
    // Retrieve available books with alphabetical sorting and limit the result
    $totalAvailableBooks = getTotalAvailableBooksCountByGenre($genre);
    $totalAvailablePages = ceil($totalAvailableBooks / $booksPerPage);
    $alphabeticalAvailableBooks = getAvailableBookByGenre($genre, $offset, $booksPerPage);

    // Retrieve borrowed books with alphabetical sorting and limit the result
    $totalBorrowedBooks = getTotalBorrowedBooksCountByGenre($genre);
    $totalBorrowedPages = ceil($totalBorrowedBooks / $booksPerPage);
    $alphabeticalBorrowedBooks = getBorrowedBookByGenreAZ($genre, $offset, $booksPerPage);
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="genre-intro"><br><br>
            <div class="title1">Sort by:</div>
            <a class="book-links" href="genre-Popularity.php?Genre=<?php echo urlencode($genre)?>">Popularity</a>
            <a class="book-links" href="genre-AZ.php?Genre=<?php echo urlencode($genre)?>">Alphabetically</a><br><br><br>
        </div>
        <div class="title1"><h2><?php echo "Available ".$genre." Manga";?></h3></div>
        <div class="flex-container">
            <?php foreach($alphabeticalAvailableBooks as $items){ ?>
                <div class="box">
        ...
        <div class="pagination">
            <?php for ($i = 1; $i <= $totalAvailablePages; $i++) { ?>
                <a href="genre-AZ.php?Genre=<?php echo urlencode($genre); ?>&page=<?php echo $i; ?>"
        ...
        <div class="flex-container">
            <?php foreach($alphabeticalBorrowedBooks as $items){ ?>
                <div class="box">
                    <a href="info.php?Title=<?php echo urlencode($items['Title']);?>">
                        <div>
                            <?php   
